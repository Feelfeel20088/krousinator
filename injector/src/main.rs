...
use winapi::um::{processthreadsapi::OpenProcess, winnt::PROCESS_ALL_ACCESS}; // 0xFFFFFFFF
...
fn main() {
    ...
    unsafe {
        ...
        if process == NULL {
            println!(
                "{} failed to get handle to the process: {}",
                PLU,
                GetLastError()
            );
            return;
        ...
        println!(
            "{} Buffer Allocated to process memory with readwrite permission: {:#?}",
            PLU, buffer
        );

        if buffer == null_mut() {
            println!(
                "{} Failed to allocate buffer: error: {}",
                PLU,
                GetLastError()
            );
            return;
        ...
        if kernel32 == null_mut() {
            println!(
                "{} Failed to get handle to kernel32.dll. Error: {}",
                PLU,
                GetLastError()
            );
            CloseHandle(process);
        ...
        if kernel32_addr.is_null() {
            println!(
                "{} Failed to get address of LoadLibrary. Error: {:#?}",
                PLU, kernel32_addr
            );
            return;
        }
        ...
        if thread.is_null() {
            println!(
                "{} Failed to get handle to the thread: {}",
                PLU,
                GetLastError()
            );
            return;
